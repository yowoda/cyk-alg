...
impl SymbolSpec {
        ...
        match self.get_symbol_id(text) {
            Some(id) => {
                if self.symbols.get(&id).unwrap().stype() == SymbolType::Terminal {
                    return Err(SymbolSpecError::StartSymbolMustBeNonTerminal(text.to_string()));
                }
    ...
    fn add_symbols(&mut self, source: &str, stype: SymbolType) -> Result<(), SymbolSpecError> {
    ...
    fn add_symbol(&mut self, text: &str, stype: SymbolType) -> Result<SymbolId, SymbolSpecError> {
...
impl From<SymbolSpecError> for GrammarParsingError {
    fn from(err: SymbolSpecError) -> Self {
        GrammarParsingError::SymbolError(err)
    }
}
